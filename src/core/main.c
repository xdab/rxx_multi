...
int main(int argc, char **argv)
{
    ...
    int file_input = (opts.input_file[0] != '\0');
    int record = (opts.record_file[0] != '\0');
    ...
    /* Compute device center frequency and requested capture width.
     * Single-channel: tune directly to the one frequency.
     * Multi-channel: tune to the midpoint; capture rate must cover full span.
     * File input (-I): rate and center come from the recording itself. */
    ...
    int downsample = plan.downsample;

    /* True channel rate after integer decimation of the capture rate */
    int rate_channel = (int)(device.rate / (uint32_t)downsample);
    ...
    for (int i = 0; i < opts.channel_count; i++)
    {
         ...
         * capture center: always true in multi-channel mode, and in file
         * input mode a single channel may sit off-center of the recording */
    ...
    /* Single demod target for direct delivery; NULL triggers fan-out in callback */
    device.demod_target = (opts.channel_count == 1) ? &demods[0] : NULL;
